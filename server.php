...
if (isset($_POST['r_time'])) {
  ...
  if (empty($met_wie))                { array_push($reservation_errors, "No with who"); }
  if ($time_start > $time_end)        { array_push($reservation_errors, "Start time can not be later than End time"); }
  
     
  list($check_available) = mysqli_fetch_array($db->query("SELECT COUNT(*) FROM reservaties WHERE date = '{$date}' AND ( ( '{$time_start}' <= time_end AND '{$time_start}' >= time_start ) OR ( '{$time_end}' <= time_end AND '{$time_end}' >= time_start ) OR ( '{$time_start}' <= time_start AND '{$time_end}' >= time_end ));"));
      ...
      echo "hello";
          ...
          if($liqry === false) {
             ...
             echo"lukt niet1";
              ...
              $liqry->bind_param('ssssss',$date,$time_start, $time_end, $name, $lokaal, $met_wie );
              if($liqry->execute()){
